...
var CreatedOKLodopObject, CLodopIsLocal, CLodopJsState;
 ...
 function onLoadLodopCallback (tag){
/*	var event = new Event('cLodopLoadEvent');//注册事件名称
	event["data"]=tag;//进行传的参数
	event["origin"]='1135443144';//进行传的参数
	window.dispatchEvent(event);//将事件注册到window
	
*/
	 var event = new Event('cLodopLoadEvent');//注册事件名称
		event["data"]=tag;//进行传的参数
		event["origin"]='1135443144';//进行传的参数
	 window.cLodopLoadEvent(event);	 
 }

 function needInstallCLodop(){
...
function loadCLodop() {
    ...
    JS1.onload  = JS2.onload  = function()    {CLodopJsState = "complete"; onLoadLodopCallback(0)}
    JS1.onerror = JS2.onerror = function(evt) {CLodopJsState = "complete"; onLoadLodopCallback(1)}
...
function getLodop(oOBJECT, oEMBED) {
    ...
    try {
        ...
        if (needCLodop()) {
        	console.log("加载  c-Lodop");
            try {
                LODOP = getCLodop();
            } catch (err) {
            	console.log("找不到 cLodop");
            }
            ...
            if (oOBJECT || oEMBED) {
                if (isIE)
                    ...
                    LODOP = oEMBED;
                onLoadLodopCallback(0);
            } else if (!CreatedOKLodopObject) {
            	console.log("start load plugin");
